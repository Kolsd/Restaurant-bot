"""
tests/test_fiscal_repo_tenant.py

Unit tests verifying that fiscal_repo functions correctly enforce tenant
...
async def test_a_read_fn_calls_set_config_with_tenant_id():
    """
    db_get_fiscal_resolution inside tenant_scope(5) must trigger
    ...
    with patch("app.services.database.get_pool", AsyncMock(return_value=pool)):
        with tenant_scope(5):
            from app.repositories.fiscal_repo import db_get_fiscal_resolution
            await db_get_fiscal_resolution(5)
...
async def test_a_list_fn_calls_set_config_with_tenant_id():
    """
    db_get_fiscal_invoices inside tenant_scope(99) must trigger set_config with '99'.
    """
    conn = _make_conn()
    pool = _make_pool(conn)
    ...
    with patch("app.services.database.get_pool", AsyncMock(return_value=pool)):
        with tenant_scope(99):
            from app.repositories.fiscal_repo import db_get_fiscal_invoices
            await db_get_fiscal_invoices(99)
    ...
    assert len(set_config_calls) == 1
    assert set_config_calls[0].args[1] == "99"
...
async def test_b_no_scope_raises_before_pool_acquire():
    ...
    with patch("app.services.database.get_pool", AsyncMock(return_value=pool)):
        with pytest.raises(TenantNotSetError):
            from app.repositories.fiscal_repo import db_get_fiscal_resolution
            await db_get_fiscal_resolution(1)
