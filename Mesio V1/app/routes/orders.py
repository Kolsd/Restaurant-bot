import hashlib
import os
from fastapi import APIRouter, Request, HTTPException
from pydantic import BaseModel
from app.services import database as db

router = APIRouter()
WOMPI_EVENTS_SECRET = os.getenv("WOMPI_EVENTS_SECRET", "TU_SECRETO_EVENTOS")

# --- New async utility functions as requested ---

async def find_dish(dish_name: str, bot_number: str):
    """
    Busca un platillo por nombre en el menú correspondiente al bot_number.
    """
    menu = await db.db_get_menu(bot_number)
    if not menu:
        return None
    # Puede que menú esté estructurado por categorías
    dishes = []
    if isinstance(menu, dict):
        for cat_dishes in menu.values():
            if isinstance(cat_dishes, list):
                dishes.extend(cat_dishes)
    elif isinstance(menu, list):
        dishes = menu
    for dish in dishes:
        if dish_name.lower() in dish.get("name", "").lower():
            return dish
    return None

async def add_to_cart(phone: str, dish_name: str, quantity: int, bot_number: str):
    """
    Añade un platillo al carrito para el teléfono especificado,
    buscando el menú correspondiente al bot_number.
    """
    dish = await find_dish(dish_name, bot_number)
    if not dish:
        return {"success": False, "error": "Plato no encontrado"}
    # Save to cart as needed (implement according to your persistence)
    # Esto es solo un ejemplo:
    cart = db._CART.setdefault(phone, {})
    cart[dish["name"]] = cart.get(dish["name"], 0) + quantity
    return {"success": True, "dish": dish["name"], "quantity": cart[dish["name"]]}

async def remove_from_cart(phone: str, dish_name: str, bot_number: str):
    """
    Elimina un platillo del carrito para el teléfono especificado,
    buscando el menú correspondiente al bot_number.
    """
    dish = await find_dish(dish_name, bot_number)
    if not dish:
        return {"success": False, "error": "Plato no encontrado"}
    cart = db._CART.get(phone, {})
    if dish["name"] in cart:
        del cart[dish["name"]]
        return {"success": True}
    return {"success": False, "error": "Plato no estaba en el carrito"}

# ----------------- Clases y rutas ------------------

class ClearCartRequest(BaseModel):
    phone: str


@router.get("/orders")
async def list_orders():
    all_orders = await db.db_get_all_orders()
    paid = [o for o in all_orders if o["paid"]]
    total_revenue = sum(o["total"] for o in paid)
    return {
        "summary": {
            "total_orders": len(all_orders),
            "paid": len(paid),
            "pending_payment": len(all_orders) - len(paid),
            "total_revenue_cop": total_revenue
        },
        "orders": all_orders
    }


@router.get("/orders/{order_id}")
async def get_single_order(order_id: str):
    order = await db.db_get_order(order_id)
    if not order:
        raise HTTPException(status_code=404, detail="Orden no encontrada")
    return order


@router.post("/cart/clear")
async def clear_user_cart(request: ClearCartRequest):
    db._CART.pop(request.phone, None)
    return {"success": True}


@router.get("/cart/{phone}")
async def view_cart(phone: str):
    cart = db._CART.get(phone, {})
    # Simple summary, real code should print menu details.
    return {"summary": cart}


@router.post("/payment/wompi-webhook")
async def wompi_webhook(request: Request):
    body = await request.json()
    body_bytes = await request.body()
    signature_header = request.headers.get("x-event-checksum", "")
    expected_sig = hashlib.sha256(
        (body_bytes.decode() + WOMPI_EVENTS_SECRET).encode()
    ).hexdigest()

    if signature_header and signature_header != expected_sig:
        raise HTTPException(status_code=401, detail="Firma inválida")

    event = body.get("event", "")
    data = body.get("data", {})

    if event == "transaction.updated":
        transaction = data.get("transaction", {})
        if transaction.get("status") == "APPROVED":
            reference = transaction.get("reference")
            transaction_id = transaction.get("id")
            if reference:
                result = await db.db_confirm_payment(reference, transaction_id)
                if result:
                    print(f"✅ PAGO CONFIRMADO — Orden: {reference} — ${result['total']:,} COP")

    return {"status": "ok"}


@router.get("/payment/confirm")
async def payment_confirm(request: Request):
    params = dict(request.query_params)
    order_id = params.get("id", "")
    status = params.get("status", "")
    order = await db.db_get_order(order_id) if order_id else None

    if status == "APPROVED" and order:
        return {
            "message": "✅ ¡Pago exitoso!",
            "order_id": order_id,
            "total": f"${order['total']:,} COP",
            "status": "Tu pedido está siendo preparado 🍽️"
        }
    return {
        "message": "❌ Pago no completado",
        "order_id": order_id,
        "status": status
    }
