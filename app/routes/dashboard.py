...
async def auth_logout(request: Request):
    token = request.headers.get("Authorization", "").replace("Bearer ", "")
    await logout(token) # FIX APLICADO
    return {"success": True}
...
async def team_invite(request: Request, body: TeamInviteRequest):
    ...
    success = await db.db_create_user(body.username, body.password, branch["name"], role=body.role, branch_id=branch_id, parent_user=creator["username"])
