...
STEP 1 — CATALOG: Send [LINK_MENU] so they can build their order. Respond with text only (no tool call).
...
STEP 4 — PAYMENT METHOD: List EVERY payment method from [MÉTODOS_DE_PAGO] explicitly in your reply (e.g. "Puedes pagar con: • Efectivo • Tarjeta débito"). Then ask which one the customer prefers. Respond with text only (no tool call).
...
async def execute_external_action(
    ...
    if action == "delivery" and restaurant_obj and not restaurant_obj.get("parent_restaurant_id"):
        ...
        elif customer_lat is None and address:
