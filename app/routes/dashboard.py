...
def _get_admin_key() -> str:
    """
    V-01 FIX: NUNCA usar fallback hardcodeado para la ADMIN_KEY.
    Si no está configurada, el endpoint falla con error claro.
    """
    key = os.getenv("ADMIN_KEY", "")
    if not key:
        raise HTTPException(
            status_code=503,
            detail="ADMIN_KEY no configurada en variables de entorno del servidor."
        )
    return key


def _check_admin(provided_key: str):
    """Verificación de admin key con comparación segura (timing-safe)."""
    import hmac
    real_key = _get_admin_key()
    if not hmac.compare_digest(provided_key.encode(), real_key.encode()):
...
async def demo_page():
    return (STATIC / "dashboard-demo.html").read_text(encoding="utf-8")
...
# ── SUPER ADMIN — V-01 FIX: sin fallback hardcodeado ─────────────────
...
async def admin_create_user(request: CreateUserRequest):
    _check_admin(request.admin_key)
...
async def admin_create_restaurant(request: CreateRestaurantRequest):
    ...
    await db.db_create_restaurant(request.name, request.whatsapp_number, request.address,
                                   menu_dict, lat, lon, request.features)
    return {"success": True}
...
async def team_invite(request: Request, body: TeamInviteRequest):
    ...
    if creator.get("role", "owner") not in ["owner", "admin"]:
        raise HTTPException(status_code=403, detail="No autorizado")
    ...
    success = await db.db_create_user(body.username, hash_password(body.password), branch["name"],
...
async def admin_parse_menu(admin_key: str, file: UploadFile = File(...)):
    ...
    try:
        if filename.endswith(".pdf"):
            ...
            messages_content.append({"type": "image", "source": {"type": "base64", "media_type": mt,
        ...
        return {"success": True, "json_menu": json.loads(
...
async def fix_branch_ids(request: Request):
    ...
    _check_admin(body.get("admin_key", ""))
    pool = await db.get_pool()
    fixed = []
    async with pool.acquire() as conn:
...
async def fix_conversations_bot_number(request: Request):
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            body.get("bot_number", "15556293573")
        )
    return {"success": True}