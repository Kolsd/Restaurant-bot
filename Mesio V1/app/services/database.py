...
async def db_toggle_bot(phone: str, pause: bool):
    ...
    async with pool.acquire() as conn:
        ...
        """, phone, pause)
        # ─────────────────────────────────────────────
# FUNCIONES SUPER ADMIN Y MENÚ DINÁMICO
# ─────────────────────────────────────────────
...
async def db_create_restaurant(name: str, whatsapp_number: str, address: str, menu: dict):
    ...
    async with pool.acquire() as conn:
        await conn.execute("""
            ...
            name=EXCLUDED.name, address=EXCLUDED.address, menu=EXCLUDED.menu
        """, name, whatsapp_number, address, json.dumps(menu))
...
async def db_get_menu(bot_number: str) -> dict:
    restaurant = await db_get_restaurant_by_phone(bot_number)
    if restaurant and restaurant.get('menu'):
        menu = restaurant['menu']
        return json.loads(menu) if isinstance(menu, str) else menu
    return {}

async def db_get_top_dishes(bot_number: str, top_n: int = 5) -> list:
    menu = await db_get_menu(bot_number)
    all_dishes = []
    for category, dishes in menu.items():
        for dish in dishes:
            all_dishes.append({**dish, "category": category})
    return sorted(all_dishes, key=lambda x: x.get("orders", 0), reverse=True)[:top_n]