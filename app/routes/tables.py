...
async def _get_bot_number_for_user(request: Request) -> str:
    """Helper: obtiene el whatsapp_number del restaurante del usuario autenticado."""
    token    = request.headers.get("Authorization", "").replace("Bearer ", "")
    username = await verify_token(token)
    user     = await db.db_get_user(username) if username else None
    bot_number = ""
    ...
    if not bot_number:
        all_r = await db.db_get_all_restaurants()
        if all_r: bot_number = all_r[0].get("whatsapp_number", "")
    return bot_number
...
async def get_waiter_alerts(request: Request):
    ...
    bot_number = await _get_bot_number_for_user(request)
