...
async def get_restaurant_wa(request: Request) -> str:
    ...
    if not username: return "15556293573"
    user = await db.db_get_user(username)
    if not user: return "15556293573"
    if user.get("branch_id"):
        r = await db.db_get_restaurant_by_id(user["branch_id"])
        if r and r.get("whatsapp_number"): return r["whatsapp_number"]
    for r in await db.db_get_all_restaurants():
        if r["name"] == user.get("restaurant_name"): return r.get("whatsapp_number", "15556293573")
    return "15556293573"
...
async def get_table_qr(request: Request, table_id: str):
    ...
    return build_qr_html(await get_restaurant_wa(request), table["name"], table_id, width=300, branch_id=table.get("branch_id"))
