...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        """)
        # Optionally, add check constraint for allowed values in subscription_status
        await conn.execute("""
            DO $$
            BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM information_schema.check_constraints
                    WHERE constraint_name = 'restaurants_subscription_status_check'
                        AND table_name = 'restaurants'
                ) THEN
                    ALTER TABLE restaurants
                    ADD CONSTRAINT restaurants_subscription_status_check
                    CHECK (subscription_status IN ('active', 'past_due'));
                END IF;
            END
            $$;
        """)
        # Ensure column bot_paused exists in conversations table (for migrations)
        await conn.execute("""
            DO $$
            BEGIN
                IF NOT EXISTS (
                    ...
                    ALTER TABLE conversations ADD COLUMN bot_paused BOOLEAN DEFAULT FALSE;
                END IF;
            END
            $$;
        """)
